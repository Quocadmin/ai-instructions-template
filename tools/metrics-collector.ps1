...
class AIMetricsCollector {
    ...
    AIMetricsCollector() {
        $this.MetricsBaseDir = "ai-agents-works/metrics"
    ...
    [hashtable]CollectProductivityMetrics() {
        $sessionsDir = "ai-agents-works/active-sessions"
        $completedDir = "ai-agents-works/completed-sessions"
    ...
    switch ($Action) {
        ...
        "export" {
            ...
            if ([string]::IsNullOrEmpty($OutputPath)) {
                $timestamp = Get-Date -Format "yyyy-MM-dd-HH-mm-ss"
                $OutputPath = "ai-agents-works/metrics/exports/metrics-export-$timestamp.$Format"
            }
