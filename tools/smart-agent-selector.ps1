...
class SmartAgentSelector {
    ...
    [void]LoadMetrics() {
        $this.MetricsFile = "ai-agents-works/metrics/agent-performance.json"
        $this.PatternsFile = "ai-agents-works/metrics/success-patterns.json"
    ...
    [void]SetupMetricsCollection() {
        ...
        $metricsDir = "ai-agents-works/metrics"
...
function Main {
    ...
    Write-Host ""
    Write-Host ".\ai-agents-works\automation\ai-work-tracker.ps1 -Action start-session -AgentName `"$($result.recommended_agent)`" -Objective `"$TaskDescription`" -AutoContext" -ForegroundColor Green
    Write-Host ""
